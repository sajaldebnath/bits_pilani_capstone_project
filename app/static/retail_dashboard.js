...
function priorityPillClass(value) {
  ...
  if (text.includes("medium") || text.includes("steady")) return "warn";
  return "risk";
}
...
function renderSalesResult(result) {
  ...
  return `
    <div class="summary-grid">
      <div class="summary-card">
        <span class="metric-value">${result.predicted_daily_units_sold}</span>
        <span class="metric-subtle">Predicted units per day</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${result.predicted_daily_revenue}</span>
        <span class="metric-subtle">Expected daily revenue</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${result.predicted_daily_profit}</span>
        <span class="metric-subtle">Expected daily profit</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${info.shelf_capacity_utilization_pct ?? 0}%</span>
    ...
    <div class="result-card">
      ...
      <div class="summary-grid">
        <div class="summary-card">
          <span class="metric-value">${info.category_average_daily_units ?? 0}</span>
          <span class="metric-subtle">Reference average units</span>
        </div>
        <div class="summary-card">
          <span class="metric-value">${info.comparison_to_reference_pct ?? 0}%</span>
          <span class="metric-subtle">Vs reference group</span>
        </div>
        <div class="summary-card">
          <span class="metric-value">${info.profit_per_shelf_unit ?? 0}</span>
    ...
    ${recommendations.map((item) => `
      <div class="rank-card">
        <div class="rank-topline">
          ...
          <div class="pill ${priorityPillClass(item.predicted_stocking_priority)}">Score ${item.recommendation_score}</div>
        </div>
        <div class="summary-grid">
          <div class="summary-card">
            <span class="metric-value">${item.predicted_daily_units_sold}</span>
            <span class="metric-subtle">Predicted units/day</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${item.expected_daily_profit}</span>
            <span class="metric-subtle">Expected daily profit</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${item.profit_per_shelf_unit}</span>
            <span class="metric-subtle">Profit per shelf unit</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${item.operational_risk_score}</span>
            <span class="metric-subtle">Operational risk score</span>
          </div>
        </div>
        <div class="bar-group">
          <div class="bar-label"><span>Recommendation score</span><strong>${item.recommendation_score}</strong></div>
...
function historicalMarkup(payload) {
  ...
  return `
    <div class="summary-grid">
      ...
      <div class="summary-card">
        <span class="metric-value">${payload.summary_cards.avg_daily_units_sold}</span>
        <span class="metric-subtle">Average units sold</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${payload.summary_cards.avg_gross_profit}</span>
        <span class="metric-subtle">Average gross profit</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${payload.summary_cards.avg_profit_density}</span>
          ...
          <tbody>
            ${topProducts.map((row) => `
              <tr>
                ...
                <td>${row.avg_daily_units_sold}</td>
                <td>${row.avg_gross_profit}</td>
                <td>${row.avg_profit_density}</td>
                <td>${row.high_priority_share}%</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      ...
      ${categories.map((row) => `
        <div class="bar-group">
          <div class="bar-label">
            ...
            <strong>${row.avg_profit_density}</strong>
          </div>
          <div class="bar-track">
            <div class="bar-fill" style="width:${Math.min(row.avg_profit_density / 4, 100)}%"></div>
          </div>
        </div>
      `).join("")}
    </div>
...
function scenarioMarkup(payload) {
  ...
  return `
    <div class="summary-grid">
      <div class="summary-card">
        <span class="metric-value">${payload.baseline.predicted_daily_units_sold}</span>
        <span class="metric-subtle">Baseline units/day</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${payload.baseline.expected_daily_profit}</span>
        <span class="metric-subtle">Baseline daily profit</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${payload.baseline.profit_per_shelf_unit}</span>
        <span class="metric-subtle">Baseline profit density</span>
      </div>
      <div class="summary-card">
        <span class="metric-value">${payload.baseline.priority_probability_high}%</span>
    ...
    ${scenarios.map((scenario) => `
      <div class="rank-card">
        <div class="rank-topline">
          ...
          <div class="pill ${priorityPillClass(scenario.predicted_stocking_priority)}">${scenario.priority_probability_high}% High</div>
        </div>
        <div class="summary-grid">
          <div class="summary-card">
            <span class="metric-value">${scenario.predicted_daily_units_sold}</span>
            <span class="metric-subtle">Units/day</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${scenario.expected_daily_profit}</span>
            <span class="metric-subtle">Daily profit</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${scenario.delta_units_vs_baseline}</span>
            <span class="metric-subtle">Delta vs baseline units</span>
          </div>
          <div class="summary-card">
            <span class="metric-value">${scenario.delta_profit_vs_baseline}</span>
    ...
    return `
      <div class="bar-group">
        <div class="bar-label">
          ...
          <strong>${value}${escapeHtml(suffix)}${metricLabel ? ` ${escapeHtml(metricLabel)}` : ""}</strong>
        </div>
        <div class="bar-track">
          <div class="bar-fill" style="width:${Math.max((value / maxValue) * 100, 2)}%"></div>
        </div>
      </div>
    `;
  }).join("");
      ...
      <div class="rank-card">
        <div class="rank-topline">
          ...
          <div class="pill ${model.compare_mode === "live" ? "good" : "neutral"}">
            ${model.prediction ? `${escapeHtml(model.prediction)} • ${(Number(model.top_confidence || 0) * 100).toFixed(1)}%` : "Benchmark only"}
          </div>
        </div>
        <div class="summary-grid">
          <div class="summary-card">
...
function showLoading(targetId, text) {
  document.getElementById(targetId).innerHTML = `<div class="loading">${escapeHtml(text)}</div>`;
}

function showError(targetId, error) {
  const detail = typeof error === "string" ? error : JSON.stringify(error, null, 2);
  document.getElementById(targetId).innerHTML = `<div class="loading error">${escapeHtml(detail)}</div>`;
}
...
function normalizeCandidate(product) {
  return {
    ...
    product_name: product.product_name || "Demo Product",
    brand_tier: product.brand_tier || "Mid",
    promotion_type: product.promotion_type || "Unknown",
    current_price: Number(product.current_price ?? 10),
    shelf_capacity: Number(product.shelf_capacity ?? 10),
    lead_time_days: Number(product.lead_time_days ?? 2),
    marketing_spend: Number(product.marketing_spend ?? 500),
    shelf_life_days: Number(product.shelf_life_days ?? 90),
    fill_rate_pct: Number(product.fill_rate_pct ?? 95),
    supplier_delay_days: Number(product.supplier_delay_days ?? 1),
    sub_category: product.sub_category || "General",
    brand_name: product.brand_name || "Unknown",
    store_type: product.store_type || "General",
    festival_name: product.festival_name || null,
    competitor_price: Number(product.competitor_price ?? 0),
    discount_percentage: Number(product.discount_percentage ?? 0),
    footfall_index: Number(product.footfall_index ?? 0),
  };
}
...
function bindTabs() {
  document.querySelectorAll(".tab-button").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (button.dataset.tabTarget === "insights-tab") {
        await loadInsights(false);
      }
      if (button.dataset.tabTarget === "model-lab-tab") {
...
function bindSpecialButtons() {
  ...
  document.getElementById("load-scenario-marketing").addEventListener("click", () => renderScenarioTable(scenarioSets.marketing));
  document.getElementById("load-scenario-shelf-life").addEventListener("click", () => renderScenarioTable(scenarioSets.shelfLife));
...
function bootstrap() {
  ...
  renderScenarioTable(scenarioSets.promo);
}
